...
void parsecommandline(int argc, char **argv, struct behavior *b) {
  int i, j; //, k;     Unused
  //char workstring[FILENAMESIZE];     Unused
...
void printtriangle(struct mesh *m, struct behavior *b, struct otri *t)
{
  ...
  printf("triangle x%lx with orientation %d:\n", (long unsigned int) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    printf("    [0] = Outer space\n");
  } else {
    printf("    [0] = x%lx  %d\n", (long unsigned int) printtri.tri,
           printtri.orient);
  }
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (long unsigned int) printvertex,
              ...
              int firstitemcount, int alignment)
{
  ...
  if ((unsigned int) alignment > sizeof(int *)) {
                         ...
                         vertex endpoint2)
{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  //enum finddirectionresult collinear;    Unused
  float ex, ey;
  float tx, ty;
  float etx, ety;
  float split, denom;
  ...
  //collinear = finddirection(m, b, splittri, endpoint1);    Unused
