...
=======
    ros::Subscriber leftCamSub = n.subscribe("camera/left/undistorted", 1, leftCamCallback);
    ros::Subscriber rightCamSub = n.subscribe("camera/right/image", 1, rightCamCallback);
    
>>>>>>> Added undistortion middle-man node which receives images from the cameras and publishes the undistorted version to /camera/[left\right]/undistorted as an Image. In the process of figuring out fisheye undistortion. 'out_camera_data.xml' is temporarily in the robosub directory for testing purposes. This is generated by the calibration algorithm.
    string topic;
