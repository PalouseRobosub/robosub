...
{
    double psi = omega(0), phi = omega(1), theta = omega(2);
    Vector3d uz(0, 0, 1), uy(0, 1, 0), ux(1, 0, 0);
    ...
    Q = tmp * sind(theta) + (Matrix3d::Identity() - uz * uz.transpose()) *
            cosd(theta) + uz * uz.transpose();
    ...
    Q = tmp * sind(phi) + (Matrix3d::Identity() - uy * uy.transpose()) *
            cosd(phi) + uy * uy.transpose();
    ...
    Q = tmp * sind(psi) + (Matrix3d::Identity() - ux * ux.transpose()) *
            cosd(psi) + ux * ux.transpose();
...
Matrix3d r3Dv(Vector3d omega, MatrixXd uv)
{
    ...
    Vector3d ux = uv.col(0), uy = uv.col(1), uz = uv.col(2);
...
{
    Eigen::JacobiSVD<MatrixXd::PlainObject> svd(a, Eigen::ComputeThinU |
            Eigen::ComputeThinV);
